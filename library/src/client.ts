...
export class Client {
  ...
  constructor(url: URL) {
    ...
    this.connection = new Cothority.network.WebSocketConnection(
      trimmedURL,
      "drynx"
    );
  ...
  async run(
    sq: SurveyQuery
  ): Promise<{ raw: List<number>; computed: List<number> | undefined }> {
    ...
    const results = vector.content.map((text) => {
      if (text.c === undefined || text.k === undefined) {
        throw new Error(
          "an encrypted vector element is missing required fields"
        );
      }
  ...
  private compute(name: string, results: number[]): List<number> | undefined {
    switch (name) {
      ...
      case "lin_reg": {
        const d = Math.round(
          (-5 + Math.sqrt(5 * 5 - 4 * (4 - 2 * results.length))) / 2
        );
        if (d < 1) {
          throw new Error();
        }
