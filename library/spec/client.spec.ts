...
describe("client", async function() {
	...
	it("connects to server", async function() {
		...
		const ret = await client.run(new lib.SurveyQuery({
			surveyid: "test-query",
			query: new lib.Query({
					selector: ["col1"],
					operation: new lib.Operation({
						nameop: "sum",
					}),
				}),

			rosterservers: new Cothority.network.Roster({list: [cn, dps[0], dps[1]]}),
			servertodp: {[cn.address]: new lib.ServerIdentityList({content: dps})},
			idtopublic,
		}))
		console.log(ret);
	})
