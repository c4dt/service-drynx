...
export class AppComponent {
  ...
  constructor(config: ConfigService) {
    this.datasets = List(
      config.DataProviders.map(async (dp) =>
        AppComponent.fetchDataset(dp.datasetURL, dp.datasetTypesURL)
      )
    );
    this.columns = Promise.all(this.datasets).then((ret) =>
    ...
    const types = typesStr.zip(header).map(([t, name]) => {
      ...
      if (dateMatches !== null) {
        ...
        switch (dateMatches[1]) {
          case "years":
            return new ColumnDatedYears(name, date);
          case "days":
            return new ColumnDatedDays(name, date);
        }
      }
  ...
  private static getRelevantColumns(
    ...
    const firstColumns: List<
      [ColumnType, string] | undefined
    > = first.types.zip(first.header);

    const merged = datasets.shift().reduce((acc, dataset) => {
      return acc
        .zipAll(dataset.types.zip(dataset.header))
        .map(([left, right]) => {
    ...
    const filtered: List<[ColumnType, string]> = merged.filter(
      (column) => column !== undefined
    );

    return filtered.filter(([t, _]) => !(t instanceof ColumnRaw));
  }
}
